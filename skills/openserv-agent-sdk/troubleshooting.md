...
## Building a single agent instead of a multi-agent workflow

**Problem:** You built one agent with all the logic in a single capability. It works but is slow, hard to debug, and can't be composed into larger pipelines.

**Rule:** On OpenServ, ALWAYS decompose into 2-5 specialized agents in a multi-agent workflow. Even simple services benefit from at least a resolver (input normalization) and a processor. Read the `openserv-multi-agent-workflows` skill and the `openserv-workflow-architect` skill for patterns.

**Why:** Fan-out parallelism cuts pipeline time in half. Specialized agents are easier to debug (you can see which task failed). Other users can compose your agents into their own workflows. The platform is designed for multi-agent orchestration.
