...
description: "Transform a loose idea into a deployment-ready OpenServ multi-agent workflow prompt. Use when the user describes what they want to build on OpenServ and needs a complete, battle-tested prompt that accounts for all known platform quirks. Produces prompts that get as close to one-shot deployment as possible. Read reference.md for the complete platform knowledge base."
---

# OpenServ Workflow Architect

Turn a loosely-formed idea into a deployment-ready multi-agent workflow prompt. The output is a prompt you hand to an AI coding agent (Claude Code, Cursor, Codex, etc.) that builds and deploys the entire system.

**Reference files:**

- `reference.md` - Complete platform knowledge: every known quirk, required code patterns, edge topology rules, task body templates, and the deploy sequence. Read this before generating any prompt.
- `recipes/` - Domain-specific knowledge files. If a recipe matches the user's domain, inject it into the generated prompt. Each recipe contains API gotchas, data chains, and patterns that make one-shot deployment possible for that vertical.
...
4. The generated prompt is self-contained: an AI coding agent can execute it without reading the skills (all quirks are embedded).
...
**Inject the entire platform quirks section from `reference.md` here.** This is the constant section - identical for every prompt. It includes: project structure, deploy prerequisites, provision/auth, model parameters, workflow creation, edge semantics, platform orchestration model, capability patterns, data flow, generate() method, testing, performance budget, build sequence, and all required code patterns.
...
- **Key implementation notes:** [API gotchas, parsing quirks, rate limits, caching strategy]
```
...
To contribute a recipe: document every API gotcha, parsing quirk, data chain, and empty-data pattern from your build. The recipe should contain enough information that an AI coding agent can build the domain-specific parts without iteration.

### Section 8: Setup Script

The setup script creates the unified multi-agent workflow with triggers and edges. Include the full code with:

- Independent wallet generation for the setup script
- `workflows.create()` with all tasks, edges, triggers (webhook + x402)
- `client.triggers.activate()` for each trigger
- `workflow.setRunning()`
- Console output of paywall URL, webhook URL, workflow ID

### Section 9: Test Script

The test script fires the free webhook and validates output. Include:
...
- [ ] All platform quirks from reference.md are embedded in the prompt
...
- [ ] Domain-specific API gotchas are documented in section 7
