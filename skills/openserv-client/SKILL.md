...
1. **Call `provision()` first** — `provision()` must run at least once before deploying. It registers the agent, creates a wallet, and writes credentials to `.openserv.json` — including the `OPENSERV_USER_API_KEY`. The recommended agent template already calls `provision()` before `run(agent)` in `main()`, so starting the agent locally (`npm run dev` or `npx tsx src/agent.ts`) is enough.

2. **Extract `OPENSERV_USER_API_KEY` into `.env`** — The deploy CLI reads this key from `.env`, but `provision()` writes it to `.openserv.json`. You must extract it after provisioning:
...
The complete per-agent deploy sequence:

```bash
# Step 1: Provision (creates .openserv.json with credentials)
npm run dev
# Wait for "Connected" message, then Ctrl-C

# Step 2: Extract API key from .openserv.json into .env
node -e "const j=JSON.parse(require('fs').readFileSync('.openserv.json','utf8')); console.log('OPENSERV_USER_API_KEY='+j.apiKey)" >> .env
...
> **Note:** You do NOT need to get `OPENSERV_USER_API_KEY` from a dashboard. It is auto-generated by `provision()` and stored in `.openserv.json`. The extraction step above bridges provision output to deploy input.
